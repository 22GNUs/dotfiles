...
/** Sync config read for session_start — must not await before first setStatus. */
...
export default function simpleRewind(pi: ExtensionAPI) {
  ...
  pi.on("session_start", async (_event, ctx) => {
    sessionAlive = true;
    // Pin built-in footer height immediately (no await). Official status-line.ts does the
    // same: setStatus in session_start before any async work. Without this, the first paint
    // stays at 2 footer lines, then async refresh setStatus expands to 3 and the editor jumps.
    // cyber-ui is 1-line always so it never jumps; this only matters for the stock footer.
    cfg = readConfigSync();
    if (ctx.hasUI && cfg.status && cfg.enabled) {
      const approx = uniqueRecent(pointsFromSession(ctx)).length;
      safeUpdateStatus(ctx, cfg, approx, true);
    }
    await refresh(ctx);
  });
